...
* `accprofile` - SSO admin user access profile. This attribute must reference one of the following datasources: `system.accprofile.name` .
* `name` - SSO admin name.
...
fortios_system_ssoadmin can be imported using:
```sh
terraform import fortios_system_ssoadmin.labelname {{mkey}}
```
