...
func flattenFunc(values map[string]interface{}) string {
	...
	m := map[string]string{
		...
		"ipv4-address":           "",
		"ipv4-address-any":       "",
		"ipv4-classnet":          `if tmp = utils.Ipv4Read(d, fmt.Sprintf("%s.%d.` + flatten(name) + `", prefix, i), *tmp); tmp != nil {`,
		"ipv4-classnet-host":     `if tmp = utils.Ipv4Read(d, fmt.Sprintf("%s.%d.` + flatten(name) + `", prefix, i), *tmp); tmp != nil {`,
		"ipv4-classnet-any":      `if tmp = utils.Ipv4Read(d, fmt.Sprintf("%s.%d.` + flatten(name) + `", prefix, i), *tmp); tmp != nil {`,
		"ipv4-netmask":           "",
		"ipv4-netmask-any":       "",
		"ipv4-address-multicast": "",
		"ipv6-address":           "",
